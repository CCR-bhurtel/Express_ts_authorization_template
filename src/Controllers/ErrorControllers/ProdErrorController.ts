...
import { ExtendedErrorCast, ExtendedErrorValidation, ExtendedErrorJWT, ExtendedErrorToken } from './ErrorTypes';
...
class ErrorController {
    ...
    public controller = (err: any, req: Request, res: Response, next: NextFunction) => {
        let error: AppError | null = null;
        if (err.name === 'CastError' || err.kind === 'ObjectId')
            error = this.handleCastErroDB(err as ExtendedErrorCast);
        if (err.code === 11000) error = this.handleDuplicateFieldDB(err);
        if (err.name === 'ValidationError') error = this.handleValidationErrorDB(err as ExtendedErrorValidation);
        if (err.name === 'JsonWebTokenError') error = this.handleJWTError(err as ExtendedErrorJWT);
        if (err.name === 'TokenExpiredError') error = this.handleTokenExpireError(err as ExtendedErrorToken);
